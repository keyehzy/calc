...
static void s_skip(lexer *lex) { lex->buffer_ += 1; }

static const char *s_peek(lexer *lex) { return lex->buffer_; }
...
static token next_token(lexer *lex) {
  ...
  switch (s_peek(lex)[0]) {
    ...
    default:
      emit_error(error_unexpected_token, new_loc(s_peek(lex), s_peek(lex) + 1));
      break;
  }
